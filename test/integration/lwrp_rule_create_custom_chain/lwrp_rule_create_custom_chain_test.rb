# Should create the custom chain directory
describe directory('/etc/iptables.d/nat/FOO') do
  it { should exist }
end

# Should set custom iptables rule
describe file('/etc/iptables.d/nat/FOO/custom-chain-output.rule_v4') do
  it { should exist }
  its('content') { should match('--protocol icmp --jump ACCEPT') }
end

# Should set custom ip6tables rule
describe file('/etc/iptables.d/nat/FOO/custom-chain-output.rule_v6') do
  it { should exist }
  its('content') { should match('--protocol icmp --jump ACCEPT') }
end
